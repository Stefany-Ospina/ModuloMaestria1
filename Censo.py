...
for i in range(500_000):
    ...
    if len(censo) %100_000==0:
        print("creado",len(censo),"registros")
...
def busqueda_nombre(lista,elemento):
    ...
    if len(encontrados)==0:
        ...
        return encontrados
def muestra_registros(registro):
    if registro ==None:
        print("No esxiste registro con ese dato")
...
while True:
    ...
    if op =="1":
        try:
            ...
            muestra_registros(registro)
            
    elif op == "2":
        nombre = input("Introduce nombre: ").upper()
        registro=busqueda_nombre(censo,nombre)
        if registro == None:
